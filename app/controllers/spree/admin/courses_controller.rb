module Spree
  module Admin
    class CoursesController < ResourceController
      ...
      def update
        ...
        if @object.update_attributes(permitted_resource_params)
          ...
          @course.slug = @course.slug_was if @course.slug.blank?
      ...
      def destroy
        @course = Product.friendly.find(params[:id])

        begin
          # TODO: why is @course.destroy raising ActiveRecord::RecordNotDestroyed instead of failing with false result
          if @course.destroy
            flash[:success] = Spree.t('notice_messages.product_deleted')
          else
            flash[:error] = Spree.t('notice_messages.product_not_deleted', error: @course.errors.full_messages.to_sentence)
          end
      ...
      def stock
        @variants = @course.variants.includes(*variant_stock_includes)
        @variants = [@course.master] if @variants.empty?
      ...
      def location_after_save
        spree.edit_admin_product_url(@course)
      end
      ...
      def load_data
        ...
        #@courses_choices = Product.order(:name)
      end
